...
public final class RunSGANaivelySpark extends GATKSparkTool {
    ...
    public void runTool(final JavaSparkContext ctx){
        ...
        final JavaPairRDD<Long, ContigsCollection> result = readsAroundBreakpoints.mapToPair(RunSGANaivelySpark::convertReadsToFASTQ)
                                                                                  .mapToPair(entry -> RunSGANaivelySpark.performAssembly(entry, threads, runCorrectionSteps))
                                                                                  .mapToPair(entry -> new Tuple2<>(entry._1(), new ContigsCollection(entry._2())));

        result.saveAsObjectFile(outputDir);
    }
     ...
     */
    private static Tuple2<Long, File> performAssembly(final Tuple2<Long, File> fastqFilesForEachBreakPoint, final int threads, final boolean runCorrections) throws IOException, InterruptedException{
    ...
    private static File SGASerialRunner(final File rawFASTQ, final int threads, final boolean runCorrections) throws IOException, InterruptedException{
    ...
    @VisibleForTesting
    static String SGAPreprocess(final File inputFASTQFile, final File outputDirectory, final SGAModule indexer, int threads) throws IOException, InterruptedException{
    ...
    @VisibleForTesting
    static String SGACorrect(final File inputFASTAFile, final File outputDirectory, final SGAModule indexer, int threads) throws IOException, InterruptedException{
        ...
        correction.run(correctionArgs, outputDirectory);
    ...
    @VisibleForTesting
    static String SGAFilter(final File inputFASTAFile, final File outputDirectory, final SGAModule indexer, int threads) throws IOException, InterruptedException{
        ...
        filter.run(filterArgs, outputDirectory);
        return prefix+".filter.pass.fa";
    }

    @VisibleForTesting
    static String SGArmDuplicate(final File inputFASTAFile, final File outputDirectory, final SGAModule indexer, int threads) throws IOException, InterruptedException{

        final SGAModule rmdup = new SGAModule("rmdup");
        final String rmdupArgs[] = {"--threads", Integer.toString(threads), inputFASTAFile.getName()};
        rmdup.run(rmdupArgs, outputDirectory);
    ...
    @VisibleForTesting
    static String SGAFMMerge(final File inputFASTAFile, final File outputDirectory, final SGAModule indexer, int threads) throws IOException, InterruptedException{
        ...
        merge.run(mergeArgs, outputDirectory);
    ...
    @VisibleForTesting
    static String SGAAssemble(final File inputFASTAFile, final File outputDirectory, int threads) throws IOException, InterruptedException{
